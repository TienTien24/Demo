...
export default function Post({ caption, img, like, comment, view, share }) {
  ...
  return (
    <Card sx={{ maxWidth: "100%", bgcolor: "#E0F7FA", border: "2px solid #0288D1", borderRadius: "25px" }}>
      <CardHeader
        avatar={
          <Avatar
      ...
      <Box sx={{ paddingLeft: "56px" }}>
        ...
        <CardActions disableSpacing sx={{ display: "flex", gap: "60px" }}>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <IconButton aria-label="add to favorites" onClick={handleLike} className="like-button">
              {isLiked ? (
                <FavoriteIcon sx={{ fontSize: "20px", color: "#C2185B" }} />
              ) : (
                <FavoriteBorderOutlinedIcon sx={{ fontSize: "20px", color: "#FF5722" }} />
              )}
            </IconButton>
